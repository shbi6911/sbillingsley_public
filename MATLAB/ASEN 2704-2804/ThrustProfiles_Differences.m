...
subplot(2,1,2)
hold on
plot(t/1652,smooth_625(:,1:3)*4.448)
...
It_313(i) = trapz(smooth_313(:,i)*4.448);
It_375(i) = trapz(smooth_375(:,i)*4.448);
It_438(i) = trapz(smooth_438(:,i)*4.448);
It_500(i) = trapz(smooth_500(:,i)*4.448);
It_563(i) = trapz(smooth_563(:,i)*4.448);
It_625(i) = trapz(smooth_625(:,i)*4.448);
end
end

for j = 1:5
PercentDiff_313(j) = ((It_313(j)/It_313(6)));
PercentDiff_375(j) = ((It_375(j)/It_375(6)));
PercentDiff_438(j) = ((It_438(j)/It_438(6)));
PercentDiff_500(j) = ((It_500(j)/It_500(6)));
PercentDiff_563(j) = ((It_563(j)/It_563(6)));
PercentDiff_625(j) = ((It_625(j)/It_625(6)));
