...
% hold on; grid on;                           % single trajectory
% for i = 1:n
%     plot(trajectories.("time"+string(i)),trajectories.("data"+string(i))(:,1),"LineWidth",2);
% end
...
hold on; grid on;                           % single trajectory
for i = 1:n
    plot(trajectories.("time"+string(i)),trajectories.("data"+string(i))(:,2),"LineWidth",2);
end       
title ("Bottle Rocket Air Pressure");
xlabel ("Time (s)");
ylabel ("Pressure (Pa)");
xlim([0 0.3]);
legend(["250mL","313mL","375mL","438mL","500mL","563mL","625mL"]);
%hold off;
...
%pull out time and pressure data
P_all = zeros(n,6);
end_time = zeros(1,7);
for i = 1:n
    tstring = ("time"+string(i));
    dstring = ("data"+string(i));
    stp = find((trajectories.(dstring)(:,4) ~= 10),1,'first')-10; %only phase 1
    pressures.(tstring) = trajectories.(tstring)(1:stp,1);
    end_time(i) = pressures.(tstring)(stp);
    pressures.(dstring) = trajectories.(dstring)(1:stp,2);
    P_all(i,:) = polyfit(pressures.(tstring),pressures.(dstring),5);
    scatter(pressures.(tstring),polyval(P_all(i,:),pressures.(tstring)));
end
hold off;

save("PressureModel",'P_all','end_time','pressures');
...
const.m_dry =  0.10; % dry mass of bottle rocket (kg)
const.d =  0.087; % diameter of the bottle rocket (m)
const.d_t = 0.021; % diameter of the rocket throat (m)
const.V = 0.00125; % volume of empty bottle rocket (m^3)
...
if n == 1
    ...
    watervolume = linspace(0.00025,0.000625,n);
