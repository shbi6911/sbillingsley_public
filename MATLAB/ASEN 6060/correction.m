function [t,state_matrix,norms,ier] = correction(V,mu,vrb,constraint,Nmax,tol)
...
%           ier         success flag 0=convergence 1=failure
    ...
    norms = zeros(1,Nmax);     norms(N) = norm(F);
    %implement Newton's method using minimum norm solution
    while (norm(F)>tol && N <= Nmax)
        V = V - jacob'*((jacob*jacob')\F);  %update free variable vector
        ...
        %increment counter and store norm of constraint vector
        N = N+1;    norms(N) = norm(F);
    ...
    norms(norms == 0) = [];     %truncate unused values
    if norm(F)<tol
        ier = 0;
    elseif  N >Nmax
        ier = 1;
