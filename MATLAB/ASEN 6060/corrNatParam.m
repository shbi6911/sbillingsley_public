function [t,state_matrix,norms,ier] = corrNatParam(V,mu,vrb,constraint,cont,step,Nmax,tol)
...
%           cont        flag for natural parameter continuation
%                       0=none,1=x,2=y,3=z,4=T
%           step        step size for natural parameter continuation
...
%           norms       (6,7)x n vector of constraint vector over iteration
%                       will by 6x7 if no continuation, 7x7 if cont~=0
%           ier         success flag 0=convergence 1=failure
    arguments
        ...
        cont {mustBeMember(cont,[0,1,2,3,7])} =0
        step double =0
        Nmax double =100
        tol double =1e-12
    end
    ...
    if cont ~= 0                    %natural parameter continuation
        star = V(cont) + step;
        F(7) = V(cont) - star;
        jacob(7,:) = zeros(1,7);  jacob(7,cont) = 1;
    end
